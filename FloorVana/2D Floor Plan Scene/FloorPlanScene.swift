...
class FloorPlanScene: SKScene {
    ...
    // Property to hold the actual floor plan image
    ...
    override func didMove(to view: SKView) {
        ...
        // Use the passed floorPlanImage instead of loading from assets
        if let image = floorPlanImage {
            let texture = SKTexture(image: image)
            let floorPlan = SKSpriteNode(texture: texture)
            floorPlan.position = CGPoint(x: frame.midX, y: frame.midY)
            floorPlan.setScale(1.0)
            addChild(floorPlan)
            self.floorPlanNode = floorPlan
        } else {
            // Fallback to asset if no image was provided
